...
cleanup() {
    ...
    # Kill any processes on ports 8188 and 7801
...
# ComfyUI base image already has everything set up
# Check if venv exists, otherwise use system Python
if [ -f "venv/bin/activate" ]; then
    source venv/bin/activate
    echo "Using venv for ComfyUI"
fi

# Start ComfyUI with the port from the base image (8188)
python3 main.py --listen 0.0.0.0 --port 8188 --use-sage-attention &
COMFYUI_PID=$!

echo "ComfyUI started (PID: $COMFYUI_PID) on port 8188"
...
if [ ! -z "$S3_ACCESS_KEY" ]; then
    echo "  - S3 Models: ${S3_ENDPOINT}/${S3_MODELS_BUCKET}/${S3_MODELS_PATH}"
    echo "  - S3 User Data: ${S3_ENDPOINT}/${S3_USER_BUCKET}"
fi
echo ""
echo "Press Ctrl+C to stop all services"

# Wait for processes
wait $COMFYUI_PID $SWARMUI_PID
